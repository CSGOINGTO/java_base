...
public interface Map<K, V> {
     ...
     * key-value（键值对）。Map.entrySet方法返回这个Map的一个集合视图，其元素属于此类。
     * 获取一个map entry的引用的唯一办法是从这个集合视图的iterator。
     * 这些Entry对象仅在迭代期间有效;
     * 更正式地说，如果在iterator返回entry之后修改了backing map，则entry行为是未定义的，除非是通过entry上的setValue操作。
     *
     * @see Map#entrySet()
     * @since 1.2
     */
    interface Entry<K, V> {
         ...
         * 返回entry相关的value。
         * 如果这个映射在所对应的map中被删除了（通过iterator的remove操作），那么这个调用的结果是未定义的。
         *
         ...
         * 用指定的value替代该entry对应的value（可选操作）
         * 如果该映射已经从该map中删除（通过iterator的remove操作），那么这个调用的结果是未定义的。
         *
         ...
         *                                       required to, throw this exception if the entry has been
         *                                       removed from the backing map.
         *                                       可以实现，但不是必须的。如果该entry已经从map中删除，将抛出IllegalStateException
         */
        V setValue(V value);
        ...
        public static <K extends Comparable<? super K>, V> Comparator<Entry<K, V>> comparingByKey() {
            // 返回一个可序列化的Comparator对象（类&类，JAVA8新语法）
     ...
     * 返回指定的key被映射的value，或者如果对于这个指定的key在该map中没有映射的话，返回defaultValue
     *
     * @param key          the key whose associated value is to be returned
     ...
     * 如果指定的key没有对应的value或者对应到null，那么将mappingFunction返回的结果插入到该entry。
     *
