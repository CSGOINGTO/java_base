...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         *               on failure, block.
         *   SIGNAL： 该节点的后继节点是或者是即将会是阻塞的（通过park方法），因此当当前节点释放或者取消时必须unPark它的后继节点。
         *   为了避免竞争，acquire方法必须首先指定它们需要signal，然后原子性的获取，然后在失败时进入阻塞。
         *
         *   CANCELLED:  This node is cancelled due to timeout or interrupt.
         *               Nodes never leave this state. In particular,
         *               a thread with cancelled node never again blocks.
         *   CANCELLED： 该节点由于超时或中断被取消。节点永远不会离开此状态。尤其是，有取消节点的线程永远不会再次阻塞。
         *
         ...
         *   CONDITION： 该节点目前处于一个condition队列中。
         *   在传输之前，它不会被用作同步队列的节点，此时状态将被设置为0.（此值的使用与该字段的其他用途无关，但简化了机制。）
         *
         ...
         *               since intervened.
         *   PROPAGATE: 一直共享模式，可以传播给其他节点。在doReleaseShared中对此进行了设置（仅适用于头节点），
         ...
         * 返回前一个节点，如果是null的话抛出NullPointerException。
         * 当前继节点不为null时使用。null检测应该被淘汰，但是可以帮助VM
         *
        ...
        final Node predecessor() throws NullPointerException {
            // p为当前节点的前置节点
            Node p = prev;
            // 如果p为null，说明队列为空（非空队列有一个属性都为空的头节点）
            if (p == null)
     ...
     * 自旋的纳秒数而不是park的超时时间。
     * 一个粗略的估计就足以在很短的超时时间内提高响应能力。
     */
    static final long spinForTimeoutThreshold = 1000L;

    /**
     * Inserts node into queue, initializing if necessary. See picture above.
     * 将节点插入到队列，如果需要初始化的话就进行初始化
    ...
    private final boolean parkAndCheckInterrupt() {
        ...
        // 返回当前线程的中断状态，并重置中断状态。（在线程park中，有可能有其他线程对该线程进行了中断的操作，但是该线程此刻无法响应中断的请求，因此在唤醒之后，返回该线程的中断状态，以便可以响应中断）
    ...
    private boolean doAcquireNanos(int arg, long nanosTimeout)
        ...
        try {
            for (;;) {
                ...
                if (nanosTimeout <= 0L)
                    return false;
                // nanosTimeout大于spinForTimeoutThreshold的话，会进入parkNanos方法中进入阻塞
                if (shouldParkAfterFailedAcquire(p, node) &&
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&
            ...
            // 如果阻塞过程中其他线程对当前线程进行了中断操作，则响应该中断
            selfInterrupt();
    }
     ...
     * method {@link java.util.concurrent.locks.Lock#tryLock(long, TimeUnit)}.
     *
     * @param arg the acquire argument.  This value is conveyed to
     *        {@link #tryAcquire} but is otherwise uninterpreted and
     *        can represent anything you like.
